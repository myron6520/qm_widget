...
class WTCellWidget extends StatelessWidget {
  ...
  const WTCellWidget({Key? key, required this.title, this.subTitle = "", this.detail = "", this.showBottomLine = true, this.right, this.padding, this.margin, this.rightSpan, this.titleColor = QMColor.COLOR_030319, this.titleBuilder}) : super(key: key);

  @override
  Widget build(BuildContext context) => [
        [
          [
            titleBuilder?.call(title) ?? title.toText(color: titleColor, fontSize: 16.s, height: 24 / 16),
            subTitle.isNotEmpty.toWidget(
              () => subTitle.toText(color: QMColor.COLOR_BDBDBD, fontSize: 14.fs, height: 20 / 14).applyPadding(EdgeInsets.only(top: 4.s)),
            ),
          ...
          12.s.inRow,
            ...
            decoration: BoxDecoration(
              color: Colors.white,
              border: showBottomLine ? Border(bottom: BorderSide(color: QMColor.COLOR_F2F2F2, width: 0.5.s)) : null,
            )),
        (right ??
                SvgPicture.string(
                  ...
                  width: 24.s,
                  height: 24.s,
                  colorFilter: ColorFilter.mode(QMColor.COLOR_E0E0E0, BlendMode.srcIn),
                ))
